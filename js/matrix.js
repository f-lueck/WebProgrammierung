...
class Matrix {
    ...
    static getRandomShape() {
        ...
        return Matrix.getMatrixForShape(randomShape);
    }
    ...
    static draw() {
        ...
        Score.drawScore();
        actualGame.drawMatrix(gameMatrix, {x: 0, y: 0});
        actualGame.drawMatrix(player.currentShape, player.currentPosition);
    }
    ...
    static handleRotation() {
        ...
        Matrix.rotateShape(player.currentShape, direction);
        while (Matrix.isColliding(player.currentPosition, player.currentShape)) { //as long as it collides, keep on rotating
     ...
     */
    mergeShapeToGameMatrix(position) {
     ...
     */
    static isColliding(position, shape) {
        //Extract x and y positions to use them easier
        const x = position.x;
        const y = position.y;
     ...
     */
    static rotateShape(shape, direction) {
        for (let i = 0; i < shape.length; ++i) {
            for (let j = 0; j < i; ++j) {
                //The actual rotation
                [
     ...
     */
    static getMatrixForShape(shape) {
        switch (shape) {
            case "j":
